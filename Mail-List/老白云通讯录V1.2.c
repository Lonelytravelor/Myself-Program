...
void StackInitial( Stack T )
{
 ...
 //删除整个通讯录
 void DeleteAllList ( struct Message *Mess , Stack T )
 {
 	int i = 0;
	...
	while( 1 )
	{
		...
		switch( i )
		{
			...
			case 3 :
				...
				break;
			case 4:
				printf("[ 警告 ] ， 是否删除所有联系人？（确定请输入1，取消请输入0）\n");
				 scanf("%d",&K);
				if( K == 0 ){
					exit(0);
				}else{
					DeleteAllList( Mess , T );
				}
				break; 
			case 5 :
				Updata( Mess );
			...
			case 6 :
				...
				break;
			case 7 :
				printf("[ 提示 ] 是否保存所做的修改？（确定请输入1，取消请输入0）\n");
				scanf("%d",&K);
				if( K == 0 ){
					exit(0);
				}else{
